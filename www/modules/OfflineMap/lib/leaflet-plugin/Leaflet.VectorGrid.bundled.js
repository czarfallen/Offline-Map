...
// 			z: this._getZoomForUrl()	/// TODO: Maybe replicate TileLayer's maxNativeZoom
        ...
        return new Promise(function(resolve){
            that.mbTilesDB.executeSql("SELECT tile_data FROM tiles WHERE zoom_level = ? AND tile_column = ? AND tile_row = ?", [data.z, data.x, data.y], function (res) {
                var base64Data = res.rows.item(0).tile_data;
                var blob = dataURItoBlob('data:application/x-protobuf;base64,'+base64Data);
                ...
                reader.onloadend = function () {
                    var unzip = pako.inflate(reader.result);
                    var pbf = new index( unzip );
...
L.vectorGrid.protobuf = function (url, options, db) {
	return new L.VectorGrid.Protobuf(url, options, db);
};
